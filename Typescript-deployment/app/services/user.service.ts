...
export async function addProductToCart(userId: string, productId: string, amount: number) {
    const debugLogger = debug('app:user.service.ts');
    try {
        ...
        if(!cart) {
            debugLogger('Cart not found, creating new one');
...
export async function createOrder(userId: string) {
    ...
    try {
        ...
        if (userCart.items && userCart.items.length > 0) {
            ...
            debugLogger('New order created:', createOrder._id);
            ...
            logger.info('Cart is empty, nothing to move to orders');        
        }   
    
    } catch(error) {
        logger.error(`Error moving cart to orders and clearing cart: ${error}`);
        throw error;        
    }
}
...
export async function createUser({name, email, password, isAdmin}: {name: string, email: string, password: string, isAdmin:string}) {
    ...
    try {
        ...
        debugLogger('New Cart created:', cart._id);
